...
namespace DotNet.Highcharts
{
    public class Highcharts : IHtmlString
    {
        ...
        public Highcharts SetColorAxis(ColorAxis colorAxis)
        {
            if (colorAxis == null)
            {
                // got to have a placeholder so the defaults will kick in.
                throw new Exception("If you are going to call SetColorAxis, you must indicate the min, minColor and maxColor");
            }
            _ColorAxis = colorAxis;
            // test - can i change to use baseline?
            //_ColorAxis.MaxColor = "Highcharts.getOptions().colors[0]";
            return this;
        }
        ...
        /// <summary>
        ///     The Y axis or value axis. Normally this is the vertical axis, though if the chart is inverted this is the horiontal
        ///     axis.
        ...
        public HtmlString ChartScriptHtmlString()
        {
            ...
            scripts.AppendLine(!string.IsNullOrEmpty(FunctionName)
                ? string.Format("function {0}() {{", FunctionName)
        ...
        /// <summary>
        /// This is what extracts the DOTNET code into JavaScript code
        /// </summary>
        /// <returns></returns>
        public string GetOptions()
        {
            ...
            // added by Wil Dobson to support use of ColorAxis in heatmaps
            if (_Chart != null && _Chart.Type == ChartTypes.Heatmap)
            {
                if (_ColorAxis != null)
                {
                    options.AppendLine(", ");
                ...
                {
                    // if its null, AND the chart type is a heatmap, put in some generics
                    options.AppendLine(", ");
                    options.Append(@"colorAxis: { min: 0, minColor: '#FFFFFF', maxColor: Highcharts.getOptions().colors[0] }");
                }
            }
            // end by Wil Dobson to support use of ColorAxis in heatmaps
